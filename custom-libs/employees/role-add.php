<?php
     ...
     */
    if ( $Employees->validatePermission( "role-add", $userInfo->role_id ) !== true ) returnResponse( "Access denied", 403 );
    ...
    $roleId = $Employees->addRole( $request->data->title );

    if ( $roleId === false ) returnResponse( "Something was wrong", 500 );
    ...
    $History->addLog(
        ...
        " ) добавил роль ( " . $request->data->title . " )", "info", 
        $userInfo->id, null, $roleId, $employeeDetail[ "hospital_id" ]
    );
